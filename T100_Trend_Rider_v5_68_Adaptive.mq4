//+------------------------------------------------------------------+
//|                 T100_Trend_Rider_v5_68_Adaptive.mq4              |
//|                 T100 Trend Rider - Version 5.68                  |
...
//+------------------------------------------------------------------+
#property strict
#property version   "5.68"
#property description "T100 Trend Rider v5.68 - consolidated adaptive MTF + protected exits + loss containment"
...
int OnInit()
{
   ...
   Print("====================================================");
   Print("T100 Trend Rider v5.68 STARTED");
...
{
   Comment("");
   Print("T100 Trend Rider v5.68 stopped.");
}
...
{
   double maxDistance=PullbackATRMax*EntryATR_M1();
   if(maxDistance<=0.0) return(false);

   bool recentTouch=false;
   bool recentAbove=false;
   ...
   for(int shift=1;shift<=lookback;shift++)
   {
      ...
      if(MathAbs(close-ema20)<=maxDistance ||
         MathAbs(low-ema20)<=maxDistance)
         recentTouch=true;
...
bool BearPullbackDetected()
{
   ...
   for(int shift=1;shift<=lookback;shift++)
   {
      ...
      if(MathAbs(close-ema20)<=maxDistance ||
         MathAbs(high-ema20)<=maxDistance)
         recentTouch=true;
...
bool BullTriggerCandleOK()
{
   ...
   if(body<(MinTriggerBodyATR*EntryATR_M1()))
      return(false);
...
bool EntryDistanceOK(int type)
{
   ...
   return(distance<=MaxEntryDistanceATR*EntryATR_M1());
}
...
void OpenTrade()
{
   ...
   string comment="T100 Trend Rider v5.68";
...
bool BetterStop(int type,double proposed,double currentSL,double minimumStep)
{
   ...
   if(type==OP_BUY)
   {
      ...
      return(proposed>currentSL+step);
   }
...
bool ValidStopForBroker(int type,double stop)
{
   ...
   double minDist=MathMax(StopLevelPrice(),FreezeLevelPrice())+2.0*Point;
...
bool ModifyStop(int ticket,double newSL,double currentTP,double minimumStep)
{
   ...
   if(!BetterStop(type,newSL,oldSL,minimumStep))
      return(false);
...
void ManageBuy()
{
   ...
   if(UseBreakEven && profitDistance>=BreakEvenATR*atr)
   {
      ...
      ModifyStop(ticket,be,tp,BreakEvenStepPoints);
   }
      ...
      profitDistance>=ProtectedProfitATR*atr)
   {
      ...
      ModifyStop(ticket,lock,tp,BreakEvenStepPoints);
   }
      ...
      profitDistance>=TrailingStartATR*atr)
   {
      ...
      ModifyStop(ticket,trail,tp,TrailingStepPoints);
   }
}
...
void PrintStatus()
{
   Comment(
      ...
      "BE ATR ",DoubleToString(BreakEvenATR,2)," BE Step ",DoubleToString(BreakEvenStepPoints,1),
      " Lock ATR ",DoubleToString(ProtectedProfitATR,2)," Trail ATR ",DoubleToString(TrailingStartATR,2),"\n",
