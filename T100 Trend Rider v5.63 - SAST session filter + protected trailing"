//+------------------------------------------------------------------+
//|                 T100_Trend_Rider_v5_61_Adaptive.mq4               |
//|                 T100 Trend Rider - Version 5.61                  |
//|  Adaptive MTF trend scoring + quality pullback + flexible M1 entry + controlled trailing |
//+------------------------------------------------------------------+
#property strict
#property version   "5.63"
#property description "T100 Trend Rider v5.63 - SAST session filter + protected trailing"
...
input int      BrokerUTCOffsetHours     = 2;       // Set to your broker server UTC offset
...
int OnInit()
{
   ...
   Print("====================================================");
   Print("T100 Trend Rider v5.63 STARTED");
   Print("Symbol: ",Symbol()," Period: ",Period());
   Print("Adaptive trend + robust history + flexible M1 trigger + SAST session");
...
{
   Comment("");
   Print("T100 Trend Rider v5.63 stopped.");
}
...
// FILTERS
//==================================================================
// Convert broker server time to South African Standard Time (UTC+2).
// The broker offset is an input because brokers may run UTC+2/UTC+3.
datetime SASTNow()
{
...
void OpenTrade()
{
   ...
   Print("TRADE OPENED v5.63 ticket=",ticket,
...
void ManageOpenTrades()
{
   ...
   double step=MathMax(TrailingStepPoints,1.0)*Point;
   ...
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      else
      {
         ...
         {
            double be=NormalizeDouble(open-BreakEvenPlusPoints*Point,Digits);
            if(currentSL==0 || currentSL>be) { newSL=be; modify=true; }
         }
         if(UseTrailingStop && allowTrail && profit>=atr*TrailingStartATR)
         {
            double trail=NormalizeDouble(Ask+atr*TrailingATR,Digits);
            if((newSL==0 || trail<newSL) && trail<open) { newSL=trail; modify=true; }
         }
         ...
         if(modify && (currentSL==0 || newSL<currentSL) && newSL>Ask &&
            (currentSL==0 || (currentSL-newSL)>=step))
         {
            ResetLastError();
            if(!OrderModify(OrderTicket(),open,newSL,OrderTakeProfit(),0,clrNONE))
