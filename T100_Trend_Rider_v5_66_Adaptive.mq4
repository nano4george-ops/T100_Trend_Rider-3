//+------------------------------------------------------------------+
//| T100_Trend_Rider_v5_66_Adaptive.mq4                              |
//| T100 Trend Rider v5.66                                           |
//| Combined adaptive MTF trend + pullback + protected trade manager |
//+------------------------------------------------------------------+
#property strict
#property version   "5.66"
#property description "T100 Trend Rider v5.66 - quality-gated adaptive MTF + protected trailing"
...
int OnInit()
{
   ...
   LastManagementBar=0;
   Print("====================================================");
   Print("T100 Trend Rider v5.66 STARTED");
...
{
   Comment("");
   Print("T100 Trend Rider v5.66 stopped. reason=",reason);
}
...
bool M5AlignmentOK(bool bullish)
{
   if(!RequireM5Alignment) return(true);
   if(bullish) return(IsM5Bull && EMA20_M5>EMA50_M5);
   return(IsM5Bear && EMA20_M5<EMA50_M5);
}

bool M1EMA50SideOK(bool bullish)
{
   if(!RequireM1EMA50Side) return(true);
   double c=iClose(Symbol(),PERIOD_M1,1);
   double ema50=iMA(Symbol(),PERIOD_M1,SlowEMA,0,MODE_EMA,PRICE_CLOSE,1);
   return(bullish ? c>ema50 : c<ema50);
}

bool TriggerCandleQualityOK(bool bullish)
{
   ...
   if(atr<=0 || h<=l) return(false);

   double body=MathAbs(c-o);
   double range=h-l;
   double closeLocation=(c-l)/range;

   if(body < atr*MinTriggerBodyATR) return(false);

   if(bullish)
      return(c>o && closeLocation>=BullCloseLocationMin);

   return(c<o && closeLocation<=BearCloseLocationMax);
}

bool RSIQualityOK(bool bullish)
{
   if(!UseRSIQualityWindow) return(true);
   if(bullish) return(RSI_M15>=BullRSI && RSI_M15<=BullRSIMax);
...
void OpenTrade()
{
   ...
   Print("TRADE OPENED v5.66 ticket=",ticket,
...
void PrintStatus()
{
   Print("T100 v5.66 | SASTHour=",SASTHour(),
